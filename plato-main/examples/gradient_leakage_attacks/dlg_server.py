...
dlg_result_path = f"{Config().params['result_path']}/{Config().data.datasource}_({Config().trainer.model_name}sigpartgradfindcrucial)_{Config().algorithm.cost_fn}_{Config().algorithm.attack_method}_{Config().algorithm.rec_optim}"
dlg_result_headers = [
    "Iteration",
    "Loss",
    "Average Data MSE",
...
class Server(fedavg.Server):
    ...
    def _deep_leakage_from_gradients(self, weights_received):
        ...
        gt_result_path = f"{dlg_result_path}/{os.getpid()}_ground_truth.pdf"
    ...
    def run_trial(
        ...
        trial_result_path = dlg_result_path
        trial_csv_file = f"{trial_result_path}/{os.getpid()}_evals.csv"
        ...
        #     )
        # else:
        #     #在这个self._gradient_closure函数中就已经计算好dummydata的梯度了
        ...
        for iters in range(num_iters):
            ...
            with torch.no_grad():
                ...
                if iters % log_interval == 0:
                    ...
                    csv_processor.write_csv(trial_csv_file, new_row)#到这个代码为止，那个100轮改变dummydata的循环结束了
                    reconstructed_path = trial_result_path
                    self._plot_reconstructed(num_images, history,gt_data ,reconstructed_path, dm, ds)
                    #这个地方也是8000轮结束的地方
        #有个疑惑，这里的缩近是否有问题
        with torch.no_grad():
            ...
            #reconstructed_path = f"{trial_result_path}/reconstruction_iterations.png"上述是原先的代码，这里我改了，就是为了后面好生成多份文件
            #reconstructed_path = f"{trial_result_path}/reconstruction_iterations"
            for i in range(num_images):
                logging.info("Reconstructed label is %d.", history[-1][i][1])
            final_tensor = torch.stack([history[-1][i][0] for i in range(num_images)])
            #self._plot_reconstructed(num_images, history,gt_data ,reconstructed_path, dm, ds)
            final_result_path = f"{trial_result_path}/{os.getpid()}_final_attack_result.pdf"
            self._make_plot(num_images, final_tensor, None, final_result_path, dm, ds)

            # Save the tensors into a .pt file
            tensor_file_path1 = f"{trial_result_path}/tensor"
            tensor_file_path = f"{trial_result_path}/tensor/{os.getpid()}_tensors.pt"
            if not os.path.exists(tensor_file_path1):
                os.makedirs(tensor_file_path1)
            result = {
                i
                * log_interval: {
    ...
    def _reconstruction_costs(dummy, target):
        indices = torch.arange(48,len(target)-12)
        ...
        return total_costs / len(indices)
    ...
    @staticmethod
    def _plot_reconstructed(num_images, history,image_data, reconstructed_result_path, dm, ds):
        """Plot the reconstructed data."""
        #num_images:梯度所包含训练图像的数量 
        # history：存储了dummy data在不同迭代次数情况下的图像
        # image_data：存储了ground truth图像
        # for i in range(num_images):
        #     logging.info("Reconstructed label is %d.", history[-1][i][1])
        reconstructed_result_path=reconstructed_result_path
        #下面这两步是对ground truth的图像进行处理，我目前也不知道第二步为什么要进行这样的处理
        image_data = image_data.detach().clone()
        image_data.mul_(ds).add_(dm).clamp_(0, 1)
        for i in range(num_images):
            #每个图像的迭代情况都用一个表来展示
            reconstructed_result_path_subplot = f"{reconstructed_result_path}/{os.getpid()}_of_{str(i)}th.png"
            plt.figure(figsize=(15, 10))
            #先画ground truth的图像
            plt.subplot(5,10,1)
            plt.imshow(image_data[i].permute(1, 2, 0).cpu())
            plt.title('Truth image')
            plt.axis('off')
            for j in range(min(len(history), 49)):
                plt.subplot(5,10,j+2)
                plt.imshow(
                    history[j][i][0]
                    .detach()
                    .clone()
                ...
                plt.title('iter=%d' % (j * log_interval))
                plt.axis('off')
            plt.savefig(reconstructed_result_path_subplot)
            plt.close()
